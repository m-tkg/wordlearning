...
RUN yum -y install git curl bzip2 bzip2-devel gcc make openssl-devel readline-devel zlib-devel rpmdevtools yum-utils libevent-devel mysql-devel openldap-devel vim sqlite-devel mysql-server nginx
...
expose 80
ENTRYPOINT service mysqld start; service nginx start; tail -f /dev/null
